    ...
    const imageCards = list.map((p, idx) => {
      ...
      <div class="col-item col-item-reveal" style="animation-delay: ${(idx * 0.1).toFixed(1)}s" data-idx="${idx}" data-id="${escapeAttr(p.id)}" data-name="${escapeAttr(p.name)}" data-price="${escapeAttr(p.price)}" data-img="${escapeAttr(imgPath)}" data-description="${escapeAttr(p.description || '')}">
        ...
        <div class="col-item-info">
          ...
          <button type="button" class="btn-add-cart" data-add-idx="${idx}"><i class="fas fa-shopping-cart"></i> Add to cart</button>
        </div>
      </div>`;
    }).join('\n');
...
${imageCards}${emptyState}
  ...
  <script>
    (function () {
      ...
      function loadAdmin() {
        if (window.AuraAdmin) {
          showAdminIconIfReady();
          return;
        }
        if (window.__auraAdminLoading) {
          window.__auraAdminPendingIcon = true;
          return;
        }
        ...
        s.src = '../js/admin.js';
        s.onload = function () {
          window.__auraAdminLoading = false;
          showAdminIconIfReady();
          if (window.__auraAdminPendingIcon) {
            window.__auraAdminPendingIcon = false;
            showAdminIconIfReady();
          }
        };
        s.onerror = function () { window.__auraAdminLoading = false; };
